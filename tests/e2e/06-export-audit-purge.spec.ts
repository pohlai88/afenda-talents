...
import { signIn, invite, completeAssessment, withDb } from "./helpers";
...
test("a full admin round produces a complete, identity-free audit trail", async ({ page }) => {
  test.setTimeout(180_000);
  const stamp = Date.now();
  await signIn(page); // -> admin.login

  // invite.created x3, then one of each lifecycle action.
  const walkLink = await invite(page, `Walk-${stamp}`, `walk+${stamp}@example.com`);
  ...
  const [revokeId, resendId] = await withDb(async (db) => {
    ...
    return [byEmail[`rev+${stamp}@example.com`], byEmail[`res+${stamp}@example.com`]];
  });

  expect((await page.request.post(`/api/admin/invite/${revokeId}/revoke`)).ok()).toBeTruthy(); // invite.revoked
  expect((await page.request.post(`/api/admin/invite/${resendId}/resend`)).ok()).toBeTruthy(); // invite.resent
  ...
  const { events, candidates } = await withDb(async (db) => ({
    events: (await db.query('SELECT actor, action, "subjectId", meta FROM "AuditEvent"')).rows,
    candidates: (await db.query('SELECT email, "fullName" FROM "Candidate"')).rows,
  }));
...
test("purge deletes every candidate and leaves an identity-free audit trail", async ({ page }) => {
  ...
  const purgeButton = page.getByRole("button", { name: "Delete all candidate data" });
  ...
  await expect(page.getByRole("status")).toContainText(/Deleted \d+ candidate record/);

  const remaining = await withDb(async (db) => ({
    candidates: Number((await db.query('SELECT count(*) FROM "Candidate"')).rows[0].count),
    responses: Number((await db.query('SELECT count(*) FROM "Response"')).rows[0].count),
    results: Number((await db.query('SELECT count(*) FROM "Result"')).rows[0].count),
    purgeEvents: Number(
      (await db.query(`SELECT count(*) FROM "AuditEvent" WHERE action = 'data.purged'`)).rows[0]
        .count,
    ),
  }));
