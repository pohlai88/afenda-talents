...
export default async function SitesPage() {
  ...
  const [sites, definitions] = await Promise.all([
    db.administrativeSite.findMany({
      ...
      include: { _count: { select: { serviceCoverage: true, obligations: true } } },
    }),
  ...
  const rows: SiteRow[] = sites.map((site) => ({
    ...
    notes: site.notes ?? "",
    customFields: objectValue(site.customFields),
    counterparties: site._count.serviceCoverage,
    obligations: site._count.obligations,
  }));

  return (
    <AfendaPageFrame width="wide">
      <PageHeader eyebrow="Corporate Administration" title="Sites" description="Model the locations where obligations, service providers and administrative risk actually operate." />
      <CorporateNav />
