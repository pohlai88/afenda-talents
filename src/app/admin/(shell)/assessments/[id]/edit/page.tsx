...
/**
 * Visual builder entry point (D18 §8). ADMIN only — VIEWERs are read-only across the
 * whole app, so this route never renders the builder for them.
 *
 * `Assessment.draftDocument` is nullable: a freshly published assessment has none until
 * someone starts a new draft. That state gets its own screen rather than a blank
 * builder, so the "start editing again" action is explicit.
 */
			...
			<div className="mx-auto flex w-full max-w-2xl min-w-0 flex-col gap-6 p-6">
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>
							This assessment is archived and read-only. Duplicate it from the list if you need
