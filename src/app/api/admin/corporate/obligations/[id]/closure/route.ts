...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const closure = await db.$transaction(async (tx) => {
      ...
      if (lifecycleManaged) {
        const generalLine = await tx.administrativeObligationLine.findFirst({ where: { obligationId, code: "GENERAL" }, select: { nextDueDate: true } });
        ...
        await tx.administrativeObligation.update({
          ...
          data: { status: effectiveNow ? "ENDED" : "ACTIVE", endDate: effective, nextDueDate: visibleNextDue, autoRenew: false, renewalDate: null },
        });
      } else {
        await tx.administrativeObligation.update({ where: { id: obligationId }, data: { endDate: effective, autoRenew: false, renewalDate: null } });
      }
  ...
  try {
    const result = await db.$transaction(async (tx) => {
      ...
      const [openDueItems, pendingApprovals, unreconciledPayments, unresolvedReconciliationItems] = await Promise.all([
        ...
        tx.administrativePayment.count({
          where: {
            dueItem: { obligationId },
      ...
      const updated = await tx.administrativeClosure.update({ where: { id: closure.id }, data: { status: "CLOSED", closedAt: new Date(), closedById: session.userId } });
      if (closure.lifecycleManaged) {
        ...
        await tx.administrativeObligation.update({ where: { id: obligationId }, data: { nextDueDate: null, autoRenew: false, renewalDate: null } });
      }
