...
export function CustomFieldList({ fields, isAdmin }: { fields: CorporateCustomFieldDefinitionDto[]; isAdmin: boolean }) {
  ...
  function clearFilters() { setSearch(""); setScopeFilter("ALL"); setStatusFilter("ALL"); }
  ...
  const desktop = <Table><TableHeader><TableRow><TableHead>Record type</TableHead><TableHead>Key</TableHead><TableHead>Label</TableHead><TableHead>Type</TableHead><TableHead>Rules</TableHead><TableHead>Status</TableHead>{isAdmin ? <TableHead className="w-10" /> : null}</TableRow></TableHeader><TableBody>{filteredFields.map((field) => <TableRow key={field.id}><TableCell>{corporateStatusLabel(field.scope)}</TableCell><TableCell className="font-mono text-xs">{field.key}</TableCell><TableCell className="font-medium">{field.label}</TableCell><TableCell>{corporateStatusLabel(field.dataType)}</TableCell><TableCell className="text-muted-foreground">{[field.required ? "Required" : null, field.showInList ? "List" : null].filter(Boolean).join(" · ") || "Optional"}</TableCell><TableCell><Badge variant={field.isActive ? "default" : "secondary"}>{field.isActive ? "Active" : "Inactive"}</Badge></TableCell>{isAdmin ? <TableCell className="text-right"><AfendaRowActions label={field.label} actions={[{ label: field.isActive ? "Deactivate" : "Reactivate", disabled: busyId === field.id, onSelect: () => void toggle(field) }]} /></TableCell> : null}</TableRow>)}</TableBody></Table>;

  const mobile = <ul className="flex flex-col gap-3">{filteredFields.map((field) => <li key={field.id} className="flex flex-col gap-3 rounded-lg border p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-medium">{field.label}</p><p className="font-mono text-xs text-muted-foreground">{field.scope.toLowerCase()} · {field.key}</p></div><div className="flex items-center gap-1"><Badge variant={field.isActive ? "default" : "secondary"}>{field.isActive ? "Active" : "Inactive"}</Badge>{isAdmin ? <AfendaRowActions label={field.label} actions={[{ label: field.isActive ? "Deactivate" : "Reactivate", disabled: busyId === field.id, onSelect: () => void toggle(field) }]} /> : null}</div></div><p className="text-sm text-muted-foreground">{corporateStatusLabel(field.dataType)}{field.required ? " · required" : ""}</p></li>)}</ul>;

  return (
    <AfendaSection title="Configured fields" description="Custom fields extend scalar metadata. Keep Sites, Counterparties and other relationships in their first-class relationship models.">
      {fields.length === 0 ? <AfendaEmptyState title="No custom fields configured" description="Create a custom field when organisation-specific structured metadata is missing from a Corporate Administration record." /> : <div className="flex flex-col gap-4"><AfendaFilterToolbar search={search} onSearchChange={setSearch} searchPlaceholder="Search custom fields…" resultCount={filteredFields.length}><AfendaSelectFilter ariaLabel="Filter custom fields by record type" value={scopeFilter} onValueChange={(value) => setScopeFilter(value as ScopeFilter)} options={[{ value: "ALL", label: "All record types" }, { value: "COUNTERPARTY", label: "Counterparty" }, { value: "SITE", label: "Site" }, { value: "OBLIGATION", label: "Obligation" }, { value: "DUE_ITEM", label: "Due item" }, { value: "PAYMENT", label: "Payment" }]} /><AfendaSelectFilter ariaLabel="Filter custom fields by status" value={statusFilter} onValueChange={(value) => setStatusFilter(value as StatusFilter)} options={[{ value: "ALL", label: "All statuses" }, { value: "ACTIVE", label: "Active" }, { value: "INACTIVE", label: "Inactive" }]} />{hasFilters ? <Button type="button" size="sm" variant="ghost" onClick={clearFilters}>Clear filters</Button> : null}</AfendaFilterToolbar>{filteredFields.length === 0 ? <AfendaEmptyState title="No custom fields found" description="No custom fields match the current search and filters." compact /> : <AfendaResponsiveDataView desktop={desktop} mobile={mobile} />}</div>}
    </AfendaSection>
  );
}
