...
export function DueItemPanel({ dueItem, dueFields, paymentFields, isAdmin }: {
  ...
  async function save() {
    ...
    try {
      ...
      toast.success("Due item updated."); setEditOpen(false); router.refresh();
    } catch (error) { toast.error(error instanceof Error ? error.message : "Could not update due item"); }
  ...
  return (
    <Card>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>Update due item</DialogTitle><DialogDescription>Add invoice evidence, adjust the expected amount, mark a dispute, or populate newly configured fields.</DialogDescription></DialogHeader><div className="grid gap-4 sm:grid-cols-2">
      ...
      </div><DialogFooter><Button variant="outline" onClick={() => setEditOpen(false)}>Cancel</Button><Button disabled={busy || !dueDate || !periodLabel} onClick={() => void save()}>{busy ? "Saving…" : "Save due item"}</Button></DialogFooter></Dialog>
    </Card>
  );
}
function Field({ label, id, children, className = "" }: { label: string; id: string; children: React.ReactNode; className?: string }) { return <div className={`space-y-2 ${className}`}><Label htmlFor={id}>{label}</Label>{children}</div>; }
