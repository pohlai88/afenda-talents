...
export type ObligationRegisterField = {
  id: string;
  key: string;
  label: string;
};
...
export function ObligationRegister({ rows, listFields }: { rows: ObligationRegisterRow[]; listFields: ObligationRegisterField[] }) {
  ...
  const categoryOptions = useMemo(() => {
    ...
    return [{ value: "ALL", label: "All categories" }, ...categories.map((value) => ({ value, label: value.replaceAll("_", " ") }))];
  }, [rows]);
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      const matchesAttention = attentionFilter === "ALL" || (attentionFilter === "NONE" ? row.attentionState == null : row.attentionState === attentionFilter);
      const matchesCategory = categoryFilter === "ALL" || row.category === categoryFilter;
      return matchesSearch && matchesStatus && matchesAttention && matchesCategory;
    });
      ...
      <TableBody>
        {filteredRows.map((row) => (
          <TableRow key={row.id}>
            ...
            <TableCell><Link className="font-medium underline-offset-4 hover:underline" href={`/admin/corporate/obligations/${row.id}`}>{row.title}</Link><p className="text-xs text-muted-foreground">{row.organization} · {row.counterpartyName}</p></TableCell>
      ...
      {filteredRows.map((row) => (
        <li key={row.id} className="rounded-lg border p-4">
          ...
          <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
            <div><p className="text-xs text-muted-foreground">Counterparty</p><p>{row.counterpartyName}</p></div>
            <div><p className="text-xs text-muted-foreground">Expected</p><p>{formatMoney(row.currency, row.expectedAmount)}</p></div>
            <div><p className="text-xs text-muted-foreground">Next attention</p><p>{row.nextAttentionDate ?? "—"}</p></div>
            <div>{row.attentionState ? <CorporateStatusBadge status={row.attentionState} /> : null}</div>
          </div>
        </li>
      ))}
    </ul>
  ...
  return (
    <div className="flex flex-col gap-4">
      <AfendaFilterToolbar search={search} onSearchChange={setSearch} searchPlaceholder="Search obligations…" resultCount={filteredRows.length}>
        ...
        <AfendaSelectFilter
          ariaLabel="Filter obligations by status"
          value={statusFilter}
        ...
        />
        <AfendaSelectFilter
          ariaLabel="Filter obligations by attention state"
          ...
          options={[
            ...
            { value: "NONE", label: "No next attention" },
          ]}
        />
        {hasFilters ? <Button type="button" size="sm" variant="ghost" onClick={clearFilters}>Clear filters</Button> : null}
      </AfendaFilterToolbar>
      {filteredRows.length === 0 ? <p className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">No obligations match the current filters.</p> : <AfendaResponsiveDataView desktop={desktop} mobile={mobile} />}
    </div>
  );
}
