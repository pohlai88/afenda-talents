    ...
    <AfendaSection title="Relationship graph" description="The obligation may span many sites and involve many counterparties in distinct roles." actions={isAdmin ? <div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={beginAddSite}>Link site</Button><Button size="sm" variant="outline" onClick={beginAddParty}>Link party</Button></div> : undefined}>
      <div className="grid gap-5 lg:grid-cols-2">
        <section aria-labelledby="obligation-sites-title" className="rounded-lg border p-4">
          ...
          {sites.length === 0 ? <div className="mt-4"><AfendaEmptyState compact title="No sites linked" description="Link one or more locations so the obligation appears in site context." /></div> : <ul className="mt-4 divide-y">{sites.map((site) => <li key={site.id} className="flex items-start justify-between gap-3 py-3 first:pt-0 last:pb-0"><div><Link href={`/admin/corporate/sites/${site.id}`} className="font-medium underline-offset-4 hover:underline">{site.name}</Link><p className="font-mono text-xs text-muted-foreground">{site.code} · {corporateStatusLabel(site.type)}</p></div><div className="flex flex-col items-end gap-2"><div className="flex flex-wrap items-center justify-end gap-1">{site.scopeRole ? <Badge variant="outline">{corporateStatusLabel(site.scopeRole)}</Badge> : null}<Badge variant={site.isActive ? "default" : "secondary"}>{site.isActive ? "Active" : "Inactive"}</Badge></div>{isAdmin ? <div className="flex flex-wrap justify-end gap-2">
    ...
    {site.isActive
      ...
      : <Button type="button" size="sm" variant="outline" disabled={updatingKey === site.id} onClick={() => void setSiteLinkActive(site, true)}>Reactivate</Button>}
        ...
        <section aria-labelledby="obligation-parties-title" className="rounded-lg border p-4">
          ...
          {parties.length === 0 ? <div className="mt-4"><AfendaEmptyState compact title="No parties linked" description="The primary compatibility party will be backfilled by the migration; add other roles here." /></div> : <ul className="mt-4 divide-y">{parties.map((party) => <li key={`${party.counterpartyId}-${party.roleCode}`} className="flex items-start justify-between gap-3 py-3 first:pt-0 last:pb-0"><div><Link href={`/admin/corporate/counterparties/${party.counterpartyId}`} className="font-medium underline-offset-4 hover:underline">{party.name}</Link><p className="font-mono text-xs text-muted-foreground">{party.code}</p></div><div className="flex flex-col items-end gap-2"><div className="flex flex-wrap items-center justify-end gap-1"><Badge variant="secondary">{corporateStatusLabel(party.roleCode)}</Badge>{party.isPrimary ? <Badge variant="outline">Primary</Badge> : null}<Badge variant={party.isActive ? "default" : "secondary"}>{party.isActive ? "Active" : "Inactive"}</Badge></div>{isAdmin ? <div className="flex flex-wrap justify-end gap-2">
