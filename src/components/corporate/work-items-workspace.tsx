...
/** Tabs previously read OPEN / MINE / UNASSIGNED — the raw filter tokens. */
...
export function WorkItemsWorkspace({workItems,users,currentUserId,isAdmin,today}:Props) {
  ...
  const filtered=useMemo(()=>workItems.filter(item=>{
    ...
    if(q&&!`${item.title} ${corporateStatusLabel(item.sourceType)} ${item.ownerName??""}`.toLowerCase().includes(q))return false;
  ...
  return <div className="flex flex-col gap-5">
    ...
    <Card><CardHeader><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><CardTitle>Administrative work queue</CardTitle><CardDescription>Accountable work derived from administrative records and deterministic operational signals.</CardDescription></div>{isAdmin?<Button onClick={sync} disabled={busy==="sync"}>{busy==="sync"?"Syncing…":"Sync operational work"}</Button>:null}</div></CardHeader><CardContent className="flex flex-col gap-3">
      <div className="flex flex-col gap-2 md:flex-row md:items-center"><Input type="search" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search work items, source or owner" aria-label="Search administrative work items"/><div className="flex flex-wrap gap-2">{(["OPEN","MINE","UNASSIGNED","OVERDUE","ALL"] as const).map(value=><Button key={value} size="sm" variant={view===value?"default":"outline"} onClick={()=>setView(value)}>{VIEW_LABEL[value]}</Button>)}</div></div>
