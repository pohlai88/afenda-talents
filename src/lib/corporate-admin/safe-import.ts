...
export const corporateImportPayloadSchema = z.object({
  rows: z.array(corporateImportRowSchema).min(1).max(200),
}).strict();

export const corporateImportCommitSchema = corporateImportPayloadSchema.extend({
...
export type CorporateImportChange = {
  field: string;
...
export function parseCorporateImportText(text: string): { rows: CorporateImportRow[]; errors: string[] } {
  ...
  for (let index = 1; index < lines.length; index += 1) {
    ...
    try {
      const candidate = {
        rowNumber: index + 1,
        obligationCode: source.obligation_code,
        lineCode: source.line_code,
        lineName: source.line_name || undefined,
        lineType: source.line_type || undefined,
        expectedAmount: number(source.expected_amount),
        currency: source.currency || undefined,
        recurring: bool(source.recurring),
        recurrenceInterval: number(source.recurrence_interval),
        recurrenceUnit: source.recurrence_unit ? source.recurrence_unit.toUpperCase() : undefined,
        firstDueDate: source.first_due_date || undefined,
        nextDueDate: source.next_due_date || undefined,
        invoiceRequired: bool(source.invoice_required),
        paymentTermsDays: number(source.payment_terms_days),
        startDate: source.start_date || undefined,
        endDate: source.end_date || undefined,
        notes: source.notes || undefined,
        siteCodes: source.site_codes ? source.site_codes.split(/[;,]/).map((item) => item.trim()).filter(Boolean) : [],
      };
      const parsed = corporateImportRowSchema.safeParse(candidate);
      if (!parsed.success) errors.push(`Row ${index + 1}: ${parsed.error.issues.map((issue) => issue.message).join("; ")}`);
      else rows.push(parsed.data);
    } catch (error) {
      errors.push(`Row ${index + 1}: ${error instanceof Error ? error.message : "Could not parse row"}`);
    }
  }
  return { rows, errors };
}
