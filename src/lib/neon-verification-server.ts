...
export async function getAnonymousJwt(): Promise<string> {
  ...
  const response = await fetch(`${env.VERIFICATION_AUTH_BASE}/token/anonymous`, {
    method: "GET",
  ...
  const token =
    (headerToken && headerToken.split(".").length === 3
      ? headerToken
      : null) ?? tokenFromJson(body);

  if (!response.ok || !token) {
    throw new Error(
      `Unable to obtain verification service JWT (${response.status}): ${
        text.slice(0, 500) || "no token returned"
      }`,
    );
  }
...
export async function callVerificationRpc(
  name: string,
  payload: Record<string, unknown>,
) {
  const jwt = await getAnonymousJwt();
  return fetch(`${env.VERIFICATION_DATA_API_BASE}/rpc/${name}`, {
    method: "POST",
    headers: {
